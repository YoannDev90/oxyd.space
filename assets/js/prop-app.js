...
function setDot(ip,status){
  var i=S.findIndex(function(s){return s.ip===ip});
  if(i<0)return;
  var c=d3.select(dotCircles.nodes()[i]);
...
function drawDots(data){
  ...
  var dotData=dotsG.selectAll("g").data(S).join("g").attr("transform",function(d){
...
(function () {
  ...
  function buildGrid(items){
    var resultsTitle = i18nDict.results_title || 'Results';
    return '<div class="results-header"><h2>' + resultsTitle + '</h2></div><div class="results-grid">'
  ...
  function renderResults(done,total){
    var resultsTitle = i18nDict.results_title || 'Results';
    var ok=done.filter(function(d){return d.ok}).length;
    var pct=Math.round(ok/total*100);
    var sorted=done.sort(function(a,b){return (a.ms||9999)-(b.ms||9999)});
    var cards=sorted.map(function(j){
      var cls=j.ms?(j.ms<100?'fast':j.ms>500?'slow':''):'';
    ...
    out.innerHTML=
      '<div class="results-header"><h2>'+resultsTitle+'</h2><span style="font-family:var(--mono);font-size:13px;color:var(--muted)">'+ok+'/'+total+'</span></div>'
  ...
  var domain,rtype,go,out;

  async function check(){
    var d=domain.value.trim().toLowerCase().replace(/^https?:\/\//,'').split('/')[0];
    if(!d)return;
    go.disabled=true;
    resetDots();
    var resultsTitle = i18nDict.results_title || 'Results';
    out.innerHTML='<div class="results-header"><h2>'+resultsTitle+'</h2><span style="font-family:var(--mono);font-size:13px;color:var(--muted)">0/'+S.length+'</span></div><div class="results-grid"></div>';
    var t0=performance.now();
    var done=[];
    await Promise.allSettled(S.map(function(s){
      return fetch('https://dnsrobot.net/api/dns-query',{
        method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({domain:d,recordType:rtype.value,dnsServer:s.ip,timeout:5000})
      }).then(function(r){return r.json()}).then(function(j){
        var success=j.status==='success';
  ...
  async function boot() {
    var i18n = await initI18n('propagation');
    i18nDict = i18n.dict;
    ...
    domain.addEventListener('keydown',function(e){if(e.key==='Enter')check()});

    Promise.all([
      fetch('/assets/dns.json').then(function(r){return r.json()}),
      d3.json('https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json')
    ]).then(function(pair){
      var servers=pair[0],world=pair[1];
      gMain.append("g")
